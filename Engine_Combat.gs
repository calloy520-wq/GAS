...
function buildDefaultSystemPrompt(isNsfwMode, backLocked) {
  ...
  // 2. 根據模式動態覆寫或擴充專屬欄位
  const _visibleState = {
    "衣服": "衣著與裸露狀態(12字內，純外觀，如：紅衣半褪、酥胸袒露、要描寫奶子)",
...
3. ★【禁止重複賜予】：玩家或 NPC 【原本就擁有、裝備中、或只是在劇情中拿出來展示/使用】的物品，【絕對禁止】再次寫入 items_gained！items_gained 僅限於「首次從外界或他人處取得的全新物品」。若違反此律，將導致道具無限複製的災難！
4. ★【煉成／打造/煉丹專屬系統鐵律】：玩家若僅以對話描述「丟進火堆」「隨手煉製」「開爐」等煉成意圖，但本回合並未收到系統下方的「★【天道審判 - 煉成】」裁決區塊，代表玩家尚未透過正式煉成介面操作，此時【絕對禁止】判定煉成成功、【絕對禁止】輸出任何 items_gained，只能描寫玩家「需先備妥素材並開啟煉製」的提示。唯有收到該裁決區塊時才可依其結果輸出 items_gained。
5. ★【煉成成品禁止轉手】：本回合若收到「★【天道審判 - 煉成】」裁決區塊，當回合新煉出的成品【絕對禁止】在同一回合內透過 items_transferred 轉贈、交付或被任何在場 NPC 拿走，成品必須留在玩家行囊，待玩家於【下一回合】另行明確做出贈禮動作才可轉移。
